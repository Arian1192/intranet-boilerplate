...
- **D4 — Pool de responsables.** El picker del live usa nombres cortos propios (p.ej. "Alex González", "Carlos Pego") distintos de los nombres completos de Team (`team/data/people.ts` → "Alejandro Gonzalez", "Carlos Pego Muñoz"). **Recomendación:** pool **local** `personas.ts` (mantiene el módulo autocontenido, patrón del proyecto) sembrado con los nombres cortos que muestra el live, en vez de importar `team/data`.
- **D5 — Exports = MOCK FIEL (acordado con el coordinador).** Los 3 botones (PDF Ventas / PDF Explotación / Excel) presentes e idénticos al live en posición/etiqueta; **inertes o con aviso "Próximamente"**. No generan fichero ni replican el layout del PDF. Como el repo no tiene sistema de toast, el aviso será un texto efímero inline con `aria-live` (o botones inertes si el coordinador prefiere cero-ruido). **Delta consciente documentado aquí y a repetir en la descripción de la PR**, igual que el adjunto de Incidencias.
