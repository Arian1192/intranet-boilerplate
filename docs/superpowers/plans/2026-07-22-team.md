# Team (`/personal`) — Plan de implementación

> **Para agentes de trabajo:** SUB-SKILL REQUERIDO: Usa superpowers:subagent-driven-development (recomendado) o superpowers:executing-plans para implementar este plan tarea a tarea. Los pasos usan sintaxis de checkbox (`- [ ]`) para seguimiento.

**Objetivo:** Reemplazar el `TeamShell` placeholder por un calco pixel-perfect del live `/personal`: Equipo (Directorio + Organigrama), Calendario y Fichas.

**Arquitectura:** Implementar primero la capa de datos (people, absences, fichas/companies) con helpers deterministas, luego extender el `SegmentedControl` compartido para el tono oscuro usado en Equipo, y finalmente construir la composición a nivel de página siguiendo el patrón de carpetas existente (`src/features/team/{data,components,pages}`). Usar rutas anidadas de React Router bajo `/personal` y tabs de módulo de `AppLayout`.

**Stack tecnológico:** React, TypeScript, Tailwind CSS, React Router, Vitest, Testing Library, primitivos existentes de `@/components/ui`.

## Restricciones globales

- Calco pixel-perfect del live; no se inventan botones ni features.
- El modelo de datos espeja tablas de Supabase con FKs; denormalizaciones documentadas en comentarios del código.
- TDD: un test antes de cada implementación; commits pequeños y por unidad.
- El live es solo lectura; nunca mutarlo.
- Sin clases `brand-*` en grises/negros; el tono activo oscuro usa `#44444C` (`bg-[#44444C]`) o `slate-800` según corresponda.
- Lint `--max-warnings 0`; `tsc` clean (ES2020, no `Array.prototype.at()`).
- Mantener la baseline verde: 112 archivos de test, 295 tests pasando.
- No tocar `src/features/incidencias`, Configuración ni sus specs.

## Estructura de archivos

| Archivo | Responsabilidad |
|---|---|
| `src/features/team/data/types.ts` | Interfaces `Person`, `Absence`, `AbsenceType`, `Company`, `CompanyId`, `Ficha` |
| `src/features/team/data/people.ts` | Seed de 26 personas + helpers (`allPeople`, `searchPeople`, `departmentsList`, `directReports`, `orgRoots`, `personLabel`) |
| `src/features/team/data/people.test.ts` | Tests de contrato de datos para helpers de personas |
| `src/features/team/data/absences.ts` | Seed de 3 ausencias de julio 2026 + helpers (`absencesForMonth`, `absenceDays`, `todaysAbsence`, `totalDaysForPerson`) |
| `src/features/team/data/absences.test.ts` | Tests de contrato de datos para helpers de ausencias |
| `src/features/team/data/fichas.ts` | Seed de empresas, seed de 26 fichas + helpers (`fichaFor`, `filterFichas`, `companies`, `totalMonthlyCost`, `costRankingByPerson`) |
| `src/features/team/data/fichas.test.ts` | Tests de contrato de datos para fichas/empresas |
| `src/components/ui/SegmentedControl.tsx` | Extender con prop opcional `tone?: 'light' \| 'dark'` (default `'light'`) |
| `src/components/ui/SegmentedControl.test.tsx` | Añadir test para la clase activa del tono oscuro |
| `src/features/team/TeamShell.tsx` | Nuevo shell: `AppLayout` con tabs de módulo Equipo/Calendario/Fichas + `<Outlet />` |
| `src/features/team/TeamShell.test.tsx` | El shell renderiza tabs y los children del outlet |
| `src/features/team/components/PersonCard.tsx` | Tarjeta para la grilla de Directorio |
| `src/features/team/components/PersonCard.test.tsx` | Renderiza iniciales/foto, puestos, departamento, contactos faltantes, link de manager |
| `src/features/team/components/DirectorioToolbar.tsx` | Input de búsqueda + select de departamento |
| `src/features/team/components/DirectorioToolbar.test.tsx` | Los controles disparan callbacks |
| `src/features/team/components/OrgRow.tsx` | Fila recursiva para Organigrama |
| `src/features/team/components/OrgRow.test.tsx` | Badge, indentación, icono de email, link de Ficha |
| `src/features/team/pages/EquipoPage.tsx` | Toggle Directorio/Organigrama + composición |
| `src/features/team/pages/EquipoPage.test.tsx` | Toggle, búsqueda, filtro de departamento, filas Org |
| `src/features/team/components/CalendarLegend.tsx` | Checkbox "Solo aprobadas" + leyenda de 5 dots |
| `src/features/team/components/CalendarLegend.test.tsx` | Callback del checkbox + dots de leyenda |
| `src/features/team/components/TeamCalendarGrid.tsx` | Tabla persona × día con barras de ausencia |
| `src/features/team/components/TeamCalendarGrid.test.tsx` | Sombreado de fin de semana, resaltado de hoy, barras, columna de días |
| `src/features/team/pages/CalendarioPage.tsx` | Selector de mes + banner + leyenda + grid |
| `src/features/team/pages/CalendarioPage.test.tsx` | Banner "Hoy", navegación de mes, filtro de aprobadas |
| `src/features/team/components/FichasFilterPanel.tsx` | `+ Nueva persona`, selects Empresa/Tipo |
| `src/features/team/components/FichasFilterPanel.test.tsx` | Callbacks para los selects |
| `src/features/team/components/FichaListRow.tsx` | Fila de la lista Fichas: nombre + tipo/puestos + dots de empresa |
| `src/features/team/components/FichaListRow.test.tsx` | Renderiza tipo/puestos y dots |
| `src/features/team/components/CostByCompanyCard.tsx` | Tarjeta "COSTE POR EMPRESA" |
| `src/features/team/components/CostByCompanyCard.test.tsx` | Barras de progreso + moneda |
| `src/features/team/components/CostByPersonCard.tsx` | Tarjeta de ranking "COSTE POR PERSONA" |
| `src/features/team/components/CostByPersonCard.test.tsx` | Orden del ranking + moneda |
| `src/features/team/components/CostDashboard.tsx` | Total + dos tarjetas |
| `src/features/team/components/CostDashboard.test.tsx` | Valor exacto del total |
| `src/features/team/components/FichaDetailPanel.tsx` | Formulario de detalle con tabs, badges, banner tras edición real |
| `src/features/team/components/FichaDetailPanel.test.tsx` | Datos sembrados de Alba Gelabert, banner al editar, tabs placeholder |
| `src/features/team/pages/FichasPage.tsx` | Lista master-detail + dashboard |
| `src/features/team/pages/FichasPage.test.tsx` | Filtros, selección `?p=`, totales del dashboard |
| `src/app/router.tsx` | Reemplazar ruta plana `/personal` por rutas anidadas |

## Tarea 1: Modelo de datos de personas + helpers

**Archivos:**
- Crear: `src/features/team/data/types.ts`
- Crear: `src/features/team/data/people.ts`
- Crear: `src/features/team/data/people.test.ts`

**Interfaces:**
- Consume: ninguno.
- Produce: `Person` interfaz; helpers `allPeople`, `searchPeople`, `departmentsList`, `directReports`, `orgRoots`, `personLabel`; FK de manager documentada.
...
Ejecutar: `npx vitest run src/features/team/data/people.test.ts`
...
git commit -m "feat(team): datos de personas model and helpers"
```

## Tarea 2: Modelo de datos de ausencias + helpers

**Archivos:**
- Crear: `src/features/team/data/absences.ts`
- Crear: `src/features/team/data/absences.test.ts`

**Interfaces:**
- Consume: `Person` from `types.ts`, `people` from `people.ts`.
- Produce: `Absence`, `AbsenceType`; helpers `absencesForMonth`, `absenceDays`, `todaysAbsence`, `totalDaysForPerson`.
...
Ejecutar: `npx vitest run src/features/team/data/absences.test.ts`
Esperado: FAIL.
...
git commit -m "feat(team): datos de ausencias model and helpers"
```

## Tarea 3: Modelo de datos de fichas y empresas + helpers

**Archivos:**
- Crear: `src/features/team/data/fichas.ts`
- Crear: `src/features/team/data/fichas.test.ts`

**Interfaces:**
- Consume: `Person` from `types.ts`, `people` from `people.ts`.
- Produce: `CompanyId`, `Company`, `Ficha`; helpers `fichaFor`, `filterFichas`, `companies`, `totalMonthlyCost`, `costRankingByPerson`.
...
describe('datos de fichas', () => {
  it('tiene 26 fichas, 4 contratado and 22 freelance', () => {
...
Ejecutar: `npx vitest run src/features/team/data/fichas.test.ts`
Esperado: FAIL.
...
git commit -m "feat(team): modelo de datos de fichas y empresas"
```

## Tarea 4: Extender SegmentedControl con tono oscuro

**Archivos:**
- Modificar: `src/components/ui/SegmentedControl.tsx`
- Modificar: `src/components/ui/SegmentedControl.test.tsx`

**Interfaces:**
- Consume: existing `SegmentedControl` API.
- Produce: new optional `tone?: 'light' | 'dark'` (default `'light'`), active dark class `bg-[#44444C] text-white`.
...
Ejecutar: `npx vitest run src/components/ui/SegmentedControl.test.tsx`
Esperado: FAIL — prop `tone` no reconocida.
...
git commit -m "feat(ui): tono oscuro de SegmentedControl para el toggle de Team"
```

## Tarea 5: TeamShell con tabs reales del módulo

**Archivos:**
- Modificar: `src/features/team/TeamShell.tsx`
- Crear: `src/features/team/TeamShell.test.tsx`
- Modificar: `src/app/router.tsx` (nested routes; exact change in Task 14)

**Interfaces:**
- Consume: `AppLayout` module tabs API.
- Produce: `TeamShell` renders `AppLayout` with tabs `[{label:'Equipo',href:'/personal'},{label:'Calendario',href:'/personal/calendario'},{label:'Fichas',href:'/personal/fichas'}]` and `<Outlet />`.
...
Ejecutar: `npx vitest run src/features/team/TeamShell.test.tsx`
Esperado: FAIL.
...
git commit -m "feat(team): TeamShell con tabs reales del módulo"
```

## Tarea 6: Componentes PersonCard y DirectorioToolbar

**Archivos:**
- Crear: `src/features/team/components/PersonCard.tsx`
- Crear: `src/features/team/components/PersonCard.test.tsx`
- Crear: `src/features/team/components/DirectorioToolbar.tsx`
- Crear: `src/features/team/components/DirectorioToolbar.test.tsx`

**Interfaces:**
- Consume: `Person` from `data/types`, `Avatar` from `@/components/ui`, `personLabel` helper.
- Produce: `PersonCard` renders a card linking to `/personal/fichas?p=<id>`; `DirectorioToolbar` exposes search + department callbacks.
...
Ejecutar: `npx vitest run src/features/team/components/PersonCard.test.tsx src/features/team/components/DirectorioToolbar.test.tsx`
Esperado: FAIL.
...
git commit -m "feat(team): PersonCard y DirectorioToolbar"
```

## Tarea 7: Vista Directorio de EquipoPage

**Archivos:**
- Crear: `src/features/team/pages/EquipoPage.tsx`
- Crear: `src/features/team/pages/EquipoPage.test.tsx`

**Interfaces:**
- Consume: `PersonCard`, `DirectorioToolbar`, `SegmentedControl`, `allPeople`, `searchPeople`, `departmentsList`.
- Produce: `EquipoPage` renders Directorio grid when `view === 'directorio'`.
...
Ejecutar: `npx vitest run src/features/team/pages/EquipoPage.test.tsx`
Esperado: FAIL.
...
git commit -m "feat(team): vista Directorio de EquipoPage"
```

## Tarea 8: Componente OrgRow y vista Organigrama de EquipoPage

**Archivos:**
- Crear: `src/features/team/components/OrgRow.tsx`
- Crear: `src/features/team/components/OrgRow.test.tsx`
- Modificar: `src/features/team/pages/EquipoPage.tsx`

**Interfaces:**
- Consume: `Person`, `Avatar`, `directReports`, `personLabel`.
- Produce: `OrgRow` recursive row; EquipoPage renders full org tree.
...
Ejecutar: `npx vitest run src/features/team/components/OrgRow.test.tsx`
Esperado: FAIL.
...
- [ ] **Paso 4: Actualizar EquipoPage para renderizar Organigrama**
...
- [ ] **Paso 5: Ejecutar los tests para verificar que pasan**

Ejecutar: `npx vitest run src/features/team/components/OrgRow.test.tsx src/features/team/pages/EquipoPage.test.tsx`
Esperado: PASS.
...
git commit -m "feat(team): vista Organigrama con OrgRow"
```

## Tarea 9: Componentes CalendarLegend y TeamCalendarGrid

**Archivos:**
- Crear: `src/features/team/components/CalendarLegend.tsx`
- Crear: `src/features/team/components/CalendarLegend.test.tsx`
- Crear: `src/features/team/components/TeamCalendarGrid.tsx`
- Crear: `src/features/team/components/TeamCalendarGrid.test.tsx`

**Interfaces:**
- Consume: `Person`, `Absence`, `absenceDays`, `totalDaysForPerson`.
- Produce: `CalendarLegend` controls `approvedOnly`; `TeamCalendarGrid` renders person × day table with absence bars.
...
Ejecutar: `npx vitest run src/features/team/components/CalendarLegend.test.tsx src/features/team/components/TeamCalendarGrid.test.tsx`
Esperado: FAIL.
...
git commit -m "feat(team): CalendarLegend y TeamCalendarGrid"
```

## Tarea 10: CalendarioPage

**Archivos:**
- Crear: `src/features/team/pages/CalendarioPage.tsx`
- Crear: `src/features/team/pages/CalendarioPage.test.tsx`

**Interfaces:**
- Consume: `CalendarLegend`, `TeamCalendarGrid`, `absencesForMonth`, `todaysAbsence`, `allPeople`.
- Produce: `CalendarioPage` with month selector, today banner, legend, grid, footer note.
...
Ejecutar: `npx vitest run src/features/team/pages/CalendarioPage.test.tsx`
Esperado: FAIL.
...
## Tarea 11: Lista de Fichas y tarjetas de costes

**Archivos:**
- Crear: `src/features/team/components/FichasFilterPanel.tsx`
- Crear: `src/features/team/components/FichasFilterPanel.test.tsx`
- Crear: `src/features/team/components/FichaListRow.tsx`
- Crear: `src/features/team/components/FichaListRow.test.tsx`
- Crear: `src/features/team/components/CostByCompanyCard.tsx`
- Crear: `src/features/team/components/CostByCompanyCard.test.tsx`
- Crear: `src/features/team/components/CostByPersonCard.tsx`
- Crear: `src/features/team/components/CostByPersonCard.test.tsx`

**Interfaces:**
- Consume: `Ficha`, `Company`, `companies`, `costRankingByPerson`, `formatCurrency`, `ProgressBar`.
- Produce: reusable list row, filter panel, cost dashboard cards.
...
Ejecutar: `npx vitest run src/features/team/components/FichasFilterPanel.test.tsx src/features/team/components/FichaListRow.test.tsx src/features/team/components/CostByCompanyCard.test.tsx src/features/team/components/CostByPersonCard.test.tsx`
Esperado: FAIL.
...
git commit -m "feat(team): fila de Fichas, filtros y tarjetas de costes"
```

## Tarea 12: Componente FichaDetailPanel

**Archivos:**
- Crear: `src/features/team/components/FichaDetailPanel.tsx`
- Crear: `src/features/team/components/FichaDetailPanel.test.tsx`

**Interfaces:**
- Consume: `Ficha`, `Person`, `UnderlineTabs`, `Input`, `Textarea`, `Select`, `Button`.
- Produce: `FichaDetailPanel` renders form with tabs; banner appears only after real edit.
...
Ejecutar: `npx vitest run src/features/team/components/FichaDetailPanel.test.tsx`
Esperado: FAIL.
...
git commit -m "feat(team): FichaDetailPanel con tabs y estado de edición"
```

## Tarea 13: CostDashboard y FichasPage

**Archivos:**
- Crear: `src/features/team/components/CostDashboard.tsx`
- Crear: `src/features/team/components/CostDashboard.test.tsx`
- Crear: `src/features/team/pages/FichasPage.tsx`
- Crear: `src/features/team/pages/FichasPage.test.tsx`

**Interfaces:**
- Consume: `MasterDetailList`, `FichasFilterPanel`, `FichaListRow`, `FichaDetailPanel`, `CostDashboard`, `fichas`, `companies`, `totalMonthlyCost`, `costRankingByPerson`, `filterFichas`, `allPeople`, `fichaFor`.
- Produce: `FichasPage` with URL-driven selection `?p=` and cost dashboard.
...
Ejecutar: `npx vitest run src/features/team/components/CostDashboard.test.tsx src/features/team/pages/FichasPage.test.tsx`
Esperado: FAIL.
...
git commit -m "feat(team): FichasPage con dashboard de costes"
```

## Tarea 14: Integración con el router

**Archivos:**
- Modificar: `src/app/router.tsx`
- Modificar: `src/features/modules/TeamShell.tsx` (already done; remove old import in router)

**Interfaces:**
- Consume: `TeamShell`, `EquipoPage`, `CalendarioPage`, `FichasPage`.
- Produce: nested `/personal` routes.
...
Ejecutar: `npm test`
Esperado: FAIL if router still points to old `TeamShell` import path or pages not wired.
...
Ejecutar: `npx vitest run src/features/team/TeamShell.test.tsx src/features/team/pages/EquipoPage.test.tsx src/features/team/pages/CalendarioPage.test.tsx src/features/team/pages/FichasPage.test.tsx`
Esperado: PASS.
...
git commit -m "feat(team): conectar rutas anidadas de /personal"
```
...
Ejecutar: `npm test`
Esperado: 112 test files, 295 baseline tests + new Team tests passing.
...
- [ ] **Step 3: Ejecutar chequeo de TypeScript**

Ejecutar: `npx tsc --noEmit`
Esperado: clean.

- [ ] **Step 4: Verificar que no hay clases brand-* en grises/negros**
...
- [ ] **Step 5: Commit de cualquier fix**

```bash
git add -A
git commit -m "fix(team): lint/tsc limpio"
```

## Checklist de cobertura de la especificación

| Requisito de la especificación | Tarea |
|---|---|
